#EXPERIMENT TYPE: CENTERAUTOPACEDREADING
#FEEDBACK (FOR QUESTIONS): NONE
#RESPONSE KEYS: 1 FOR FALSE, 2 FOR TRUE 
...
proc preventRepeat {{K {}}} {
  global _KR
  if {$K == {}} {set type ""} else {set type -$K}
  set pressbind [bind . <KeyPress$type>]
  if {$K != {} && $pressbind == {}} {set pressbind [bind . <KeyPress>]}
  set releasebind [bind . <KeyRelease$type>]
  if {$K != {} && $releasebind == {}} {set releasebind [bind . <KeyRelease>]}
  bind . <KeyPress$type> {
...
proc parseSentence sentence {
  ...
  for {set w 0} {$w < $NumWords} {incr w} {
    ...
    set Y($w) [expr [winfo height .c] / 2 - $UpShift + \
		   $CharHeight * ($Line($w) - 0.5 * $NumLines)]
...
proc blockPresentText {item message} {
  ...
  if {$ResultFile != {} && $item != {}} {
    set delay [expr [timeElapsed $StartTime $StopTime] / $ClockRes]
    puts $ResultFile "$Subject [split $item :] 0 - - $delay"
    flush $ResultFile
  }
}
